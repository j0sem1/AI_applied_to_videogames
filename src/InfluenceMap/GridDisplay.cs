...
public class GridDisplay : MonoBehaviour
{
	...
	void Update()
	{
		for (int yIdx = 0; yIdx < _data.Height; ++yIdx)
		{
			for (int xIdx = 0; xIdx < _data.Width; ++xIdx)
			{
				Tile tile = _data.GetValue(xIdx, yIdx);
				Color c = _neutralColor;
				if (tile.influence < -0.5f)
					c = Color.Lerp(_negativeColor, _negative2Color, -(tile.influence+0.5f)/0.5f);
				else if (tile.influence < 0)
					c = Color.Lerp(_neutralColor, _negativeColor, -tile.influence/0.5f);
				else if (tile.influence > 0.5f)
					c = Color.Lerp(_positiveColor, _positive2Color, (tile.influence-0.5f)/0.5f);
				else 
					c = Color.Lerp(_neutralColor, _positiveColor, tile.influence/0.5f);
